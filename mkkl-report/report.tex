...
　AR(1)モデル \(y_t = \theta * y_{t-1} + \epsilon_t \  where \ \epsilon_t \sim iid(0, \sigma^2)\) とする。（AR モデルそのものについては \ref{sec:org2459934} AR モデル で解説を行う）\\
...
\end{align}
　ここで仮説検定を行う。帰無仮説は 定常過程を示している式(15)であり、つまりは式(14) における \(\sigma_{\upsilon}^2 = 0\) である。対立仮説はこの逆であり、非定常であること、つまり \(\sigma_{\upsilon}^2\) である。\\
\begin{align}
...
上式における \(\hat{\sigma^2_\infty}\) とは \(\epsilon_t\) の長期変動に関する HAC 推定量 \footnote{特に Newey-West 推定量を用いられることが多い} である。\\
...
\subsection{AR モデル}
\label{sec:org2459934}

\subsection{MA モデル}
\label{sec:org026bb20}

\subsection{係数推定}
\label{sec:org01ab354}
\subsubsection{対数尤度}
\label{sec:org6bc0c24}
\subsubsection{AIC}
\label{sec:org6ca00a7}
\subsubsection{最小二乗法}
\label{sec:org189f5df}
\subsubsection{アメーバ法}
\label{sec:orgb2f87bc}
\subsection{SARIMA モデルについて}
\label{sec:org73e6e66}
\section{Clojure/ClojureScript を用いた Web 開発}
\label{sec:orgec7554c}
\subsection{Clojure によるバックエンド開発}
\label{sec:orgbaff487}
\subsubsection{Luminus Framework}
\label{sec:org9c90f60}
\subsubsection{Swagger UI}
\label{sec:org343eea6}
\subsection{ClojureScript によるフロントエンド開発}
\label{sec:orgb8f47e6}
\subsubsection{基本的な開発}
\label{sec:orgcc847b8}
\subsubsection{Reagent}
\label{sec:org0c33d48}
\subsubsection{core.async による非同期処理}
\label{sec:org0dab4bf}
\section{MKKL の開発}
\label{sec:orge4099a1}
\section{発展 : ARIMA 推定 と Random Forest による予測}
\label{sec:org16cea98}
\subsection{概要}
\label{sec:orga093961}
\subsection{実験方法}
\label{sec:orga226bab}
\subsection{実験結果}
\label{sec:orge7b8714}
\subsection{考察}
\label{sec:org92e7dc1}
\section{まとめと今後の課題}
\label{sec:orgf4ab469}
...
\section{付録}
\label{sec:org672d63f}
\subsection{このレポートにおける数式について}
\label{sec:orge342164}
このレポートにおける数式表現といくつかの基本的な用語の定義を以下に例と共に示す。\\
\subsubsection{独立同時分布と変数の補足}
\label{sec:org69a284a}
\begin{align}
y_t = \beta t + \epsilon_t \ where \ \epsilon_t \sim iid(0, \sigma^2)
\end{align}
　この式における \(where\) とは左式における変数の補足を行うことを意味しており、この場合で \(\epsilon_t\) の意味を補足している。\(iid(\mu, \sigma^2)\) とは 独立同時分布 (independent and identically distributed) を意味しており、この確率変数は他の確率変数と同一の分布を持ち、且つそれぞれが独立していることを示している。また独立同時分布において共分散、相関係数は 0 である。本レポートにおいてこの式は、平均 \(\mu\) 、 分散 \(\sigma^2\) に従う独立同時分布という意味を持っている。\\
　これと同様の概念にホワイトノイズというものがあるため、混乱を避けるためこちらも補足を行う。\\
　ホワイトノイズ \(\epsilon_t - W.N(0, \sigma^2)\) は以下の性質を持っている。\\
\begin{itemize}
\item \(E(E_t) = 0\) \\
　平均は0\\
...
\label{sec:org6b572ae}
\begin{align}
\hat{\pi}/ se(\hat{\pi})
\end{align}
　この式における \(se(\hat{\pi})\) とは \(\hat{\pi}\) の標準誤差を示している。標準誤差とは母集団からある標本を選んだ際にどの程度のばらつきが生じるかを示す指標であり、標本数が十分多いとき 母集団 の標準偏差 \(\sigma\) と 標本 の標準偏差 \(\hat{s}\) 、標本数 \(n\) を用いて\\
\begin{align}
