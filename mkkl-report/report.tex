...
　ARIMA モデルでは「非定常な」時系列データを「定常な」時系列データに変換した上で ARMA モデルに適用することになる。一般にこの階差は一次で十分であるらしいが、理論上は以降に紹介する ADF検定 や KPSS検定 をおこなうことで定常性を判定している。\\
\subsubsection{ADF検定}
\label{sec:org03b0544}
\subsubsection{KPSS検}
\label{sec:org6084231}
\subsection{AR モデル}
\label{sec:orgf393e38}
\subsection{MA モデル}
\label{sec:orgfed923d}
\subsection{係数推定}
\label{sec:org0495b6e}
\subsubsection{対数尤度}
\label{sec:orgef1460a}
\subsubsection{AIC}
\label{sec:org3aba9ec}
\subsubsection{最小二乗法}
\label{sec:org6ab7cc9}
\subsubsection{アメーバ法}
\label{sec:orgdd7cdfc}
\subsection{SARIMA モデルについて}
\label{sec:org7c2a5e3}
\section{Clojure/ClojureScript を用いた Web 開発}
\label{sec:org11afa83}
\subsection{Clojure によるバックエンド開発}
\label{sec:org3ef8049}
\subsubsection{Luminus Framework}
\label{sec:org9a22658}
\subsubsection{Swagger UI}
\label{sec:org73d7d27}
\subsection{ClojureScript によるフロントエンド開発}
\label{sec:org0bd086c}
\subsubsection{基本的な開発}
\label{sec:org269c806}
\subsubsection{Reagent}
\label{sec:org0ab6560}
\subsubsection{core.async による非同期処理}
\label{sec:org9d71931}
\section{MKKL の開発}
\label{sec:orgb52cf66}
\section{発展 : ARIMA 推定 と Random Forest による予測}
\label{sec:orgcb5fb4e}
\subsection{概要}
\label{sec:org4d88980}
\subsection{実験方法}
\label{sec:org40108af}
\subsection{実験結果}
\label{sec:org0648e58}
\subsection{考察}
\label{sec:orgd7565a0}
\section{まとめと今後の課題}
\label{sec:org08ed9d9}
